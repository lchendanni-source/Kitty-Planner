...
const T = {
  en: {
    ...
    goalName:"Goal Name", goalTarget:"Target", goalType:"Type",
    addGoal:"Add Goal 🐱", checkIn:"Check In", addAmount:"Amount",
    ...
    goalTypes:["365-Day","52-Week","12-Month","Fixed","Free"],
  ...
  zh: {
    ...
    goalName:"计划名称", goalTarget:"目标金额", goalType:"类型",
    addGoal:"添加计划 🐱", checkIn:"打卡", addAmount:"存入金额",
    saved:"已存", remaining:"剩余", noGoals:"还没有存钱计划 🐾",
    calendarView:"日历", mostSpent:"最多", leastSpent:"最少",
    days:["日","一","二","三","四","五","六"],
    months:["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"],
    goalTypes:["365天","52周","12月","定额","自由"],
...
function loadData(){ try{ const r=localStorage.getItem(KEY); if(r) return JSON.parse(r); }catch{} return {txns:[],budget:{},goals:[],lang:"en",cat:{name:"Kitty",photo:null,wearing:[],owned:[],pointsSpent:0}}; }
...
function CatSVG({ wearing=[], level=0, size=180, photo=null }) {
  ...
  if (photo) return (
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      <div style={{width:size,height:size,borderRadius:"50%",overflow:"hidden",
        border:`4px solid ${C.primary}`,boxShadow:`0 4px 20px rgba(245,183,49,0.4)`}}>
        <img src={photo} alt="cat" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
      </div>
      <div style={{position:"absolute",top:-12,left:"50%",transform:"translateX(-50%)",
        display:"flex",gap:3,justifyContent:"center",flexWrap:"wrap",maxWidth:size+40}}>
        {wearing.map(id=>{ const item=SHOP_ITEMS.find(x=>x.id===id); return item?<span key={id} style={{fontSize:size*0.19,filter:"drop-shadow(0 2px 4px rgba(0,0,0,0.25))"}}>{item.emoji}</span>:null; })}
      </div>
    </div>
  );
  ...
  return (
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      <svg viewBox="0 0 200 210" width={size} height={size} style={{overflow:"visible"}}>
        <style>{`
          @keyframes blink{0%,88%,100%{transform:scaleY(1)}94%{transform:scaleY(0.06)}}
          @keyframes bob{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}
          @keyframes tailWag{0%,100%{transform:rotate(-12deg)}50%{transform:rotate(14deg)}}
          .cb{animation:bob 3.2s ease-in-out infinite}
          .el{animation:blink 4.5s ease-in-out infinite;transform-origin:78px 108px}
          .er{animation:blink 4.5s ease-in-out infinite 0.15s;transform-origin:122px 108px}
          .ct{animation:tailWag 2.2s ease-in-out infinite;transform-origin:62px 168px}
        `}</style>
        <ellipse cx="100" cy="202" rx="48" ry="7" fill="rgba(212,149,106,0.12)"/>
        <g className="ct">
          <path d="M64 170 Q28 172 22 146 Q16 122 44 118 Q56 116 59 130 Q38 132 40 150 Q42 162 64 160 Z" fill={bc} stroke={C.paw} strokeWidth="2.5" strokeLinejoin="round"/>
        </g>
        <g className="cb">
          <ellipse cx="100" cy="162" rx="50" ry="40" fill={bc} stroke={C.paw} strokeWidth="2.5"/>
          <ellipse cx="100" cy="168" rx="30" ry="26" fill="white" opacity="0.55"/>
          <circle cx="100" cy="100" r="50" fill={bc} stroke={C.paw} strokeWidth="2.5"/>
          {/* Ears */}
          <path d="M56 66 L44 34 L78 56 Z" fill={bc} stroke={C.paw} strokeWidth="2.5" strokeLinejoin="round"/>
          <path d="M60 63 L52 42 L74 58 Z" fill="#FFB3C6" opacity="0.85"/>
          <path d="M144 66 L156 34 L122 56 Z" fill={bc} stroke={C.paw} strokeWidth="2.5" strokeLinejoin="round"/>
          <path d="M140 63 L148 42 L126 58 Z" fill="#FFB3C6" opacity="0.85"/>
          {/* Head stripes */}
          {level>1&&<>
            <path d="M84 56 Q87 49 90 56" fill="none" stroke={C.paw} strokeWidth="1.8" opacity="0.45"/>
            <path d="M100 52 Q100 45 100 52" fill="none" stroke={C.paw} strokeWidth="1.8" opacity="0.45"/>
            <path d="M110 56 Q113 49 116 56" fill="none" stroke={C.paw} strokeWidth="1.8" opacity="0.45"/>
          </>}
          {/* Eyes */}
          <g className="el"><circle cx="78" cy="108" r="11" fill="white"/><circle cx="78" cy="108" r="7.5" fill="#2D1B00"/><circle cx="75" cy="105" r="2.8" fill="white" opacity="0.95"/></g>
          <g className="er"><circle cx="122" cy="108" r="11" fill="white"/><circle cx="122" cy="108" r="7.5" fill="#2D1B00"/><circle cx="119" cy="105" r="2.8" fill="white" opacity="0.95"/></g>
          <ellipse cx="68" cy="118" rx="11" ry="7" fill="#FFB3C6" opacity="0.55"/>
          <ellipse cx="132" cy="118" rx="11" ry="7" fill="#FFB3C6" opacity="0.55"/>
          {/* Nose + mouth */}
          <path d="M96 124 L100 128 L104 124 Q100 119 96 124 Z" fill={C.accent}/>
          <path d="M100 128 Q93 135 89 132" fill="none" stroke={C.paw} strokeWidth="2" strokeLinecap="round"/>
          <path d="M100 128 Q107 135 111 132" fill="none" stroke={C.paw} strokeWidth="2" strokeLinecap="round"/>
          {/* Whiskers */}
          {[[-1,120,-0.8],[0,126,0],[1,132,0.8]].map(([d,y,a],i)=><>
            <line key={`wl${i}`} x1="58" y1={y+d*2} x2="89" y2={y} stroke={C.paw} strokeWidth="1.4" opacity="0.65"/>
            <line key={`wr${i}`} x1="111" y1={y} x2="142" y2={y+d*2} stroke={C.paw} strokeWidth="1.4" opacity="0.65"/>
          </>)}
          {/* Paws */}
          <ellipse cx="75" cy="196" rx="19" ry="12" fill={bc} stroke={C.paw} strokeWidth="2"/>
          <ellipse cx="125" cy="196" rx="19" ry="12" fill={bc} stroke={C.paw} strokeWidth="2"/>
          {[67,75,83].map(x=><circle key={x} cx={x} cy="200" r="3.8" fill="#FFB3C6"/>)}
          {[117,125,133].map(x=><circle key={x} cx={x} cy="200" r="3.8" fill="#FFB3C6"/>)}
          {/* Accessories */}
          {acc["bow"]      && <text x="100" y="60"  fontSize="26" textAnchor="middle">🎀</text>}
          {acc["crown"]    && <text x="100" y="50"  fontSize="30" textAnchor="middle">👑</text>}
          {acc["tophat"]   && <text x="100" y="50"  fontSize="28" textAnchor="middle">🎩</text>}
          {acc["beret"]    && <text x="100" y="52"  fontSize="26" textAnchor="middle">🎨</text>}
          {acc["flowerhat"]&& <text x="100" y="52"  fontSize="28" textAnchor="middle">💐</text>}
          {acc["witchhat"] && <text x="100" y="48"  fontSize="30" textAnchor="middle">🧙</text>}
          {acc["halo"]     && <text x="100" y="46"  fontSize="28" textAnchor="middle">😇</text>}
          {acc["horns"]    && <text x="100" y="50"  fontSize="26" textAnchor="middle">😈</text>}
          {acc["shades"]   && <text x="100" y="116" fontSize="28" textAnchor="middle">🕶️</text>}
          {acc["monocle"]  && <text x="114" y="114" fontSize="20" textAnchor="middle">🧐</text>}
          {acc["mask"]     && <text x="100" y="116" fontSize="26" textAnchor="middle">🎭</text>}
          {acc["nerd"]     && <text x="100" y="116" fontSize="24" textAnchor="middle">🤓</text>}
          {acc["scarf"]    && <text x="100" y="150" fontSize="28" textAnchor="middle">🧣</text>}
          {acc["cape"]     && <text x="100" y="156" fontSize="30" textAnchor="middle">🦸</text>}
          {acc["tuxedo"]   && <text x="100" y="154" fontSize="26" textAnchor="middle">🤵</text>}
          {acc["kimono"]   && <text x="100" y="154" fontSize="28" textAnchor="middle">👘</text>}
          {acc["sweater"]  && <text x="100" y="154" fontSize="26" textAnchor="middle">🧶</text>}
          {acc["wand"]     && <text x="150" y="108" fontSize="24" textAnchor="middle">⭐</text>}
          {acc["rainbow"]  && <text x="100" y="36"  fontSize="32" textAnchor="middle">🌈</text>}
          {acc["sparkles"] && <text x="152" y="76"  fontSize="22" textAnchor="middle">✨</text>}
          {acc["moon"]     && <text x="156" y="102" fontSize="22" textAnchor="middle">🌙</text>}
          {acc["balloon"]  && <text x="154" y="74"  fontSize="24" textAnchor="middle">🎈</text>}
          {acc["flower"]   && <text x="56"  y="74"  fontSize="22" textAnchor="middle">🌸</text>}
          {acc["sunflower"]&& <text x="56"  y="72"  fontSize="22" textAnchor="middle">🌻</text>}
          {acc["ribbon"]   && <text x="100" y="154" fontSize="22" textAnchor="middle">🎗️</text>}
          {acc["butterfly"]&& <text x="154" y="90"  fontSize="22" textAnchor="middle">🦋</text>}
          {acc["diamond"]  && <text x="100" y="156" fontSize="24" textAnchor="middle">💎</text>}
        </g>
      </svg>
    </div>
  );
...
export default function App() {
  ...
  const [goalForm,setGoalForm] = useState({name:"",target:"",type:0,color:0});
  ...
  function addGoal() {
    ...
    upd(d=>({...d,goals:[...d.goals,{id:Date.now(),name:goalForm.name,target:+goalForm.target,
      type:goalForm.type,color:GOAL_COLORS[goalForm.color],saved:0,history:[],created:todayStr()}]}));
    meow("save"); setShowGoalForm(false); setGoalForm({name:"",target:"",type:0,color:0});
  ...
  // Calendar month navigation state (separate from date range filter)
  ...
  function CalendarGrid() {
    ...
    return (
      <div>
        ...
        {/* Day Detail Panel — slides in below calendar when a day is selected */}
        {selectedDay&&(()=>{
  ...
  return (
    <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh",
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:95,position:"relative",zIndex:1}}>
        ...
        {tab==="budget"&&(
          <div style={{padding:"12px 13px 0"}}>
            ...
            {Object.entries(budget).length>0&&(()=>{
        ...
        {tab==="savings"&&(
          <div style={{padding:"12px 13px 0"}}>
            <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:4,marginBottom:11}}>
              {t.goalTypes.map((gt,i)=>(
                <div key={i} style={{flexShrink:0,borderRadius:14,padding:"9px 13px",textAlign:"center",minWidth:68,
                  background:[C.yellow1,"#E3F2FD","#F3E5F5","#E8F5E9","#FFF3E0"][i],
                  border:`1.5px solid ${[C.yellow2,C.blue,C.purple,C.green,C.accent][i]}55`}}>
                  <div style={{fontSize:18}}>{["🗓️","📅","📆","💰","🆓"][i]}</div>
                  <div style={{fontSize:9,color:C.text,marginTop:3,fontWeight:600}}>{gt}</div>
                </div>
              ))}
            </div>
                ...
                <div key={g.id} style={{background:"#FFFDF5",borderRadius:20,border:`2px solid ${g.color}55`,padding:"13px",marginBottom:10,boxShadow:`0 3px 10px rgba(212,149,106,0.1)`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                    <div style={{display:"flex",gap:9,alignItems:"center"}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:11,color:g.color,fontWeight:700}}>{fmtMoney(g.target)}</div>
                        <div style={{fontSize:8,color:C.textLight}}>+{Math.floor(g.target*10)} 🐾 on complete</div>
                      </div>
                    </div>
                    <div style={{display:"flex",gap:5}}>
                  ...
                  {checkinGoal===g.id&&(
                    <div style={{marginTop:10,display:"flex",gap:7,alignItems:"center"}}>
                      ...
                      <YBtn onClick={()=>checkin(g.id)}>{t.checkIn}</YBtn>
      ...
      {showAdd&&(()=>{
        // Mini calendar picker state (scoped inside render)
        ...
        return (
        <Modal onClose={()=>{ setShowAdd(false); setCalPad(""); setShowDatePicker(false); }}>
          ...
          {/* Numpad — 3 cols numbers + 1 col actions */}
          ...
          <div style={{marginBottom:9}}>
            ...
            {/* Date display bar — click to expand/collapse calendar */}
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <div style={{textAlign:"left"}}>
                  <div style={{fontSize:11,color:showDatePicker?"rgba(61,43,26,0.65)":C.textLight}}>
                    {form.date===todayStr()?"Today 今天":
                     form.date===new Date(Date.now()-86400000).toISOString().slice(0,10)?"Yesterday 昨天":""}
                  </div>
                  <div style={{fontSize:15,fontWeight:700,color:showDatePicker?C.text:C.primary2}}>
                    {form.date.slice(5).replace("-","/")} {(()=>{const d=new Date(form.date+"T00:00:00");return t.days[d.getDay()];})()} 
                  </div>
                </div>
              </div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              <div style={{marginTop:6,background:"#FFFDF5",borderRadius:14,
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
                  {pickerCells.map((d,i)=>{
                    ...
                    const isSelected=key===form.date, isToday=key===todayStr();
                    ...
                    return (
                      <div key={i} onClick={()=>{ if(isFuture) return; setForm(f=>({...f,date:key})); meow("soft"); setShowDatePicker(false); }}
                        style={{aspectRatio:"1",borderRadius:7,
                ...
                <div style={{marginTop:8,fontSize:9,color:C.textLight,textAlign:"center"}}>
                  🐾 Days with orange dots have existing transactions
                </div>
              </div>
            )}
          </div>
      ...
      {showGoalForm&&(
        <Modal onClose={()=>setShowGoalForm(false)}>
          ...
          <input value={goalForm.name} onChange={e=>setGoalForm(f=>({...f,name:e.target.value}))} placeholder="e.g. New iPhone 🐷" style={{...inpS,width:"100%",marginBottom:9}}/>
          <FLabel>{t.goalTarget}</FLabel>
          <input value={goalForm.target} onChange={e=>setGoalForm(f=>({...f,target:e.target.value}))} placeholder="$0.00" type="number" style={{...inpS,width:"100%",marginBottom:9}}/>
          <FLabel>{t.goalType}</FLabel>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:9}}>
            {t.goalTypes.map((gt,i)=>(
              <button key={i} onClick={()=>setGoalForm(f=>({...f,type:i}))} style={{
                padding:"5px 10px",borderRadius:10,border:"none",cursor:"pointer",fontSize:11,
                background:goalForm.type===i?`linear-gradient(135deg,${C.primary},${C.accent})`:C.bgSoft,
                color:goalForm.type===i?C.text:C.textMid,fontWeight:goalForm.type===i?700:400,
                border:goalForm.type===i?"none":`1.5px solid ${C.border}`}}>{gt}</button>
            ))}
          </div>
          <FLabel>Color</FLabel>
      ...
      {showSet&&(
        <Modal onClose={()=>setShowSet(false)}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            <button onClick={()=>{ if(window.confirm("Clear all data?")){ upd(()=>({txns:[],budget:{},goals:[],lang:data.lang,cat:{name:"Kitty",photo:null,wearing:[],owned:[],pointsSpent:0}})); setShowSet(false); meow("delete"); }}} style={{...setBtn,background:"#FFF0F0",border:"1.5px solid #FFD6D6",color:C.red}}>🗑️ {t.clearAll}</button>
          </div>
        </Modal>
      )}
